...
class ProjDao:
    ...
    def add_recruit_post_details(id, data):
        ...
        """
        rich_text_list = RichTextMgr.get_richtext_by_type('post', id, 'proj_recruit_detail')
        result = []
    ...
    def add_recruit_post_highlight(id, data):
        ...
        """
        rich_text = RichTextMgr.query_first({'bus_type': 'post', 'bus_id': id, 'text_type': 'proj_highlight'},
    ...
    def add_modify_time_str(data):
        ...
        if 'modify_time' in data:
            data['modify_time_str'] = time_util.timestamp2dateString(data['modify_time'])
    ...
    def list_posts_by_org(org_id, page, page_size=10):
        ...
        count = ProjRecruitPostMgr.count(filter_conditions=filter_condition, expressions=expressions)
        if page_size > 5000:
            page_size = 5000
        records = ProjRecruitPostMgr.query(filter_conditions=filter_condition, expressions=expressions, limit=page_size,
                                     offset=(page - 1) * page_size, order_list=[ProjRecruitPostMgr.model.modify_time.desc()])
        return {'total_count': count, 'datas': to_dict(records)}